...
export default function Historique() {
  ...
  useEffect(() => {
    const fetchValidations = async () => {
      try {
        ...
        // Filtrer pour ne garder que les validations du CVR connecté
        const mesValidations = response.data.filter(v => {
          ...
          // Vérifier aussi que le rôle est CVR
          const roleMatch = v.validateurRole && String(v.validateurRole).toUpperCase() === 'CVR';
  ...
  const handleExport = () => {
    ...
    saveAs(blob, `historique-verifications-${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const columns = [
    ...
    },
    {
      id: 'decision',
      label: 'Décision',
      render: (value) => <StatusChip status={value === 'VALIDE' ? 'VALIDEE_CVR' : 'REJETEE_CVR'} label={value} />,
    },
    {
      id: 'commentaire',
      label: 'Commentaire',
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Consultez toutes vos décisions de vérification"
        action={
          <ActionButton
            variant="contained"
